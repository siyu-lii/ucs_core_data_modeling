	...
	for {
		...
		case <-tic.C:
			...
			for i := 0; i < dnode.NoDp; i++ {
				//模拟部分：sampleLen=1，dimen=2，bytelen=4， MSB，随机数据
				valueList := make([]byte, dnode.DpDimen*4) // 每个数据点有2个维度，存储4字节的int32数据
				if i == 0 {
					// 模拟生成温度数据 (维度0)
					temp := utils.RandInt32(-30, 50)                            // 随机生成温度范围-30°C到50°C
				...
				dmsg.DataSet[i] = &msg.DMsgData{
					Meta: &meta.DataMeta{
						...
						DataClass: meta.DATA_CLASS_UINT,
					},
					Data: valueList,
				}
			}
